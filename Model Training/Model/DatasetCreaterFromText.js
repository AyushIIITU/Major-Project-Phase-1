async function convertToProductivityArray(input) {
    const tasks = input.split('\n');

    const result = await tasks.map(task => {
        const [text, label] = task.split(',');
        console.log(text,label);
        
        return {
            text: text?.trim(),
            label: label?.trim()?.toLowerCase() === 'productive' ? 'productive' : 'non-productive'
        };
    });
    return result;
}
...
// Example usage
const input = `"Finish writing the presentation slides for next week's meeting",Productive
...
const output =await convertToProductivityArray(input);
